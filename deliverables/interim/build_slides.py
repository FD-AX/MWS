...
def main() -> int:
    ...
    add_table(prs, "Команда и распределение задач", ["Участник", "Роль", "Зона ответственности", "Степень участия"], [
        ["Коротков Леонид (@fDAFX)", "AI Engineer — исследование и направление",
         "генерация и отбор идей: гипотезы, архитектурные решения, план экспериментов, выбор моделей и метрик; методология оценки",
         "ведущая, постоянно"],
        ["Рыженко Виктор (@RyzhenkoViktor)", "AI Engineer — реализация",
         "код и инфраструктура: конвейер ревью и тесты, бенч и фабрика синтетики, compose-контур, интеграция self-hosted модели",
         "частичная, по задачам реализации"],
        ["Сокол Тимофей (@xcmgg)", "AI Product — процессы и продукт",
         "процесс команды и курс развития, продуктовая проработка (гипотеза, MVP, критерии, риски), фронтенд аналитика, участие в коде",
         "высокая, постоянно"],
    ], note="Степень участия — на 04.09; в финальной версии уточняется по фактическому вкладу.", col_widths=[3.0, 2.0, 5.3, 1.8])
